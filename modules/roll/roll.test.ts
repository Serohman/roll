import { Roll, RollResult } from "./Roll"; // Adjust the import based on your file structure
...
describe("Roll", () => {
  ...
  describe("roll", () => {
    test("should return correct natural and modified values with default mechanic", () => {
      const result: RollResult = roll.roll();
    ...
    test("should apply modifier correctly", () => {
      const resultA: RollResult = roll.roll(3);
      const resultB: RollResult = roll.roll(-3);
    ...
    test("should allow mechanic to be overridden", () => {
      ...
      const result: RollResult = roll.roll(0, { mechanic: mockMechanic });
    ...
    test("should allow randomizer to be overridden", () => {
      ...
      const result: RollResult = roll.roll(0, { randomizer });
