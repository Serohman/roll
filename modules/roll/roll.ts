...
export class Roll {
  ...
  constructor(
    ...
    this.mechanic = defaults.mechanic || new SingleRollMechanic();
  }

  roll(modifier: number = 0, overrideDefaults: RollConfig = {}): RollResult {
    const mechanic = overrideDefaults.mechanic || this.mechanic;
    const randomizer = overrideDefaults.randomizer || this.randomizer;

    const natural = mechanic.do(this.min, this.max, randomizer);
  ...
  rollAdvantage(
    modifier: number,
    overrideDefaults: Omit<RollConfig, "mechanic"> = {}
  ): RollResult {
