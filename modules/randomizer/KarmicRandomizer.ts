...
export class KarmicRandomizer extends Randomizer {
  ...
  protected generator(min: number, max: number): number {
    let rawRoll = this.generateRawRoll();
    let biasedRoll = this.applyBias(this.biasFactor, rawRoll);

    const rollResult = this.mapRollToRange(biasedRoll, min, max);
  ...
  get recentHighRolls() {
    return this.historyQueue.filter((entry) => entry === HistoryEntry.HIGH)
      .length;
  }

  get recentLowRolls() {
    return this.historyQueue.filter((entry) => entry === HistoryEntry.LOW)
      .length;
  }
